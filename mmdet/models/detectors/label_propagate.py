...
def affinity_sparse2dense(affinity_sparse, ind_from, ind_to, n_vertices):
    ...
    for aff in affinity_sparse:
        aff = aff.view(-1).cpu()
        affinity_dense = torch.sparse.FloatTensor(torch.cat([indices, indices_id, indices_tp], dim=1),
                                        torch.cat([aff, torch.ones([n_vertices]), aff])).to_dense().cuda()
                
        list_aff_dense.append(affinity_dense)
    
    affinity_dense = torch.stack(list_aff_dense)
...
def to_transition_matrix(affinity_dense, beta, times):
    ...
    trans_mat = scaled_affinity / torch.sum(scaled_affinity, dim=1, keepdim=True) # change from 0 to 2 for batch
    # A = trans_mat
    # for _ in range(times):
    #     A = torch.matmul(A, trans_mat)
    # trans_mat = A 
...
def propagate_to_edge(x, edge, path_index=None, radius=5, beta=10, exp_times=5):
    ...
    sparse_aff = edge_to_affinity(edge_padded, path_index.path_indices)

    dense_aff = affinity_sparse2dense(sparse_aff, path_index.src_indices,
                                      path_index.dst_indices, ver_padded * hor_padded)
    dense_aff = dense_aff.view(-1, ver_padded, hor_padded, ver_padded, hor_padded)
    dense_aff = dense_aff[:, :-radius, radius:-radius, :-radius, radius:-radius]
    dense_aff = dense_aff.reshape(-1, height * width, height * width)

    #from here
    # D_ = torch.eye(dense_aff.size(1)).repeat(dense_aff.size(0), 1, 1).cuda()
    # dense_aff = dense_aff * (1 - D_)
    
    # alpha = 0.1
    # D = dense_aff / torch.sum(dense_aff, dim=2, keepdim=True)
    # I = torch.eye(dense_aff.size(1)).unsqueeze(0).cuda()

    # inv = (I - alpha * D).inverse()
    # inv = torch.where(torch.isnan(inv)|torch.isinf(inv), torch.zeros_like(inv), inv)
    
    # rw = torch.matmul((1 - alpha)*inv, x)
    # to here

    trans_mat = to_transition_matrix(dense_aff, beta=beta, times=exp_times)
    x = (x * (1 - edge.view(-1, height * width, 1))).view(x.size(0), height * width, -1)
    rw = torch.matmul(trans_mat, x)

    rw = rw.view(rw.size(0), height, width, -1).permute(0, 3, 1, 2).argmax(1, keepdim=True).float()
