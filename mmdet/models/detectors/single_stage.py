...
class SingleStageDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        self.constraint_part = bbox_head.use_boundary
        ...
        if mask_head is not None:
            ...
            if self.use_prototype:
                self.mask_head = None
                self.protonet = PrototypeNet(self.mask_head, self.num_protos, constraint_part=self.constraint_part)
    ...
    def forward_train(self,
        ...
        if self.use_prototype:            
            mask_pred, loss_part = self.protonet(pos_mask_feats, sampling_protos)
            if self.constraint_part:
                # losses['loss_part'] = loss_part * 0.1
                losses['loss_part'] = loss_part * self.constraint_part

            mask_targets = self.protonet.get_target(sampling_results, gt_masks, self.train_cfg.rcnn)

            loss_mask = self.protonet.loss(mask_pred, mask_targets) # use dice loss 
            # loss_mask = self.mask_head.loss(mask_pred.unsqueeze(1), mask_targets) # use cross entropy loss
    ...
    def simple_test(self, img, img_meta, rescale=False, gt_masks=None):
        ...
        if det_bboxes.shape[0] > 0:
            ...
            mask_feats, return_lvls = self.mask_roi_extractor(
                    x[:len(self.mask_roi_extractor.featmap_strides)], mask_rois, im_size=img.shape[-2:], return_lvls=True) 
            
            if self.use_prototype:
                mask_pred = self.protonet(mask_feats, det_protos, training=False)
