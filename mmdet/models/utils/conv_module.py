...
from .gen_conv import GenConv2d

from mmdet.ops.dcn import ModulatedDeformConvPack
...
class ConvModule(nn.Module):
    ...
    def forward(self, x, ref_feat=None, activate=True, norm=True):
        if self.activate_last:
            if isinstance(self.conv, GenConv2d):
                x = self.conv(x, ref_feat)
            else:
                x = self.conv(x)
